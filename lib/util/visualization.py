...
def create_pil_collage(images, fpath, layout=None, vlims=None):
    ...
    collage = np.concatenate([np.concatenate(images[x*cols:(x+1)*cols], axis=ca)
                        for x in range(rows)],
                       axis=ra)
    ...
    if len(images[0].shape)==2:
        ...
        im = Image.fromarray(np.uint8(np.moveaxis(collage, 0, -1)
                            ))

    # im = Image.fromarray(np.clip(orig_slice[::-1, :] + 1000, 0, 2000) / 2000 * 255)
    im = im.convert('RGB')
    im.save(fpath)
# -------------------------------------------------------------------------
