...
class PNetLin(nn.Module):
    def __init__(self,
        ...
        # Pretrained Network to extract embedding from
        self.net = net_type(pretrained=not self.pnet_rand,
                            requires_grad=self.pnet_tune)

        # linear layers that leanrs the perceptual length from embeddings
        if lpips:
    ...
    def forward(self,
        ...
        in0_input, in1_input = (self.scaling_layer(in0),
                               ...
                               if self.version=='0.1' else (in0, in1)

        # embedding from pretrained nn
        outs0, outs1 = self.net.forward(in0_input), \
                       self.net.forward(in1_input)
        ...
        # get lin layer output for lpips
        if self.lpips:
            if self.spatial:
        ...
        # return the output feature distances
        val = res[0]
...
class Dist2LogitLayer(nn.Module):
    ...
    def __init__(self,
        ...
        layers = [nn.Conv2d(5,  chn_mid,
                            1,  stride=1,
                            padding=0, bias=True),]
        layers += [nn.LeakyReLU(0.2,True),]
        layers += [nn.Conv2d(chn_mid, chn_mid,
                             1,       stride=1,
                             padding=0, bias=True),]
        layers += [nn.LeakyReLU(0.2,True),]
        layers += [nn.Conv2d(chn_mid, 1,
                             1,       stride=1,
                             padding=0, bias=True),]
        if use_sigmoid:
            layers += [nn.Sigmoid(),]
    ...
    def forward(self,d0,d1,eps=0.1):
        ...
        return self.model.forward(torch.cat((d0, d1, d0-d1, d0/(d1+eps),
                                             d1/(d0+eps)),  dim=1))
# -----------------------------------------------------------------------------
...
class L2(FakeNet):
    ...
    def forward(self,
        ...
        if self.colorspace=='RGB':
            ...
            value = util.l2(util.tensor2np(util.tensor2tensorlab(in0.data,
                                                                 to_norm=False)),
                    util.tensor2np(util.tensor2tensorlab(in1.data,
                                                         to_norm=False)),
...
class DSSIM(FakeNet):
    ...
    def forward(self,
        ...
        if self.colorspace=='RGB':
            ...
            value = util.dssim(util.tensor2np(util.tensor2tensorlab(in0.data,
                                                                    to_norm=False)),
                util.tensor2np(util.tensor2tensorlab(in1.data,
                                                     to_norm=False)),
                               range=100.).astype('float')
